...
namespace SimpleWeb {
    ...
    public:
        ...
        public:
            Resource() {
              client =  new Client<HTTPS>("graph.facebook.com", false);
            }

            void action(std::ostream& response, std::shared_ptr<ServerBase<socket_type>::Request> request) {
                std::string content;
        ...
        typedef std::pair<std::string, Resource *> connect;

        std::map<std::string,Resource *> resources;
        

  void route(std::string path, Resource * resource) {
    resources.insert(connect(path, resource));
  }
    ...
    protected:
        ...
        virtual void write_response(std::shared_ptr<socket_type> socket, std::shared_ptr<Request> request) {
        //Find path- and method-match, and generate response
        for (auto res_it: resources) {
            ...
            if(std::regex_match(request->path, sm_res, e)) {
                ...
                res_it.second->action(response, request); // res_it->second[request->method](response, request);
                //res_it.second->prueba(response, request); // res_it->second[request->method](response, request);
                //response << "prueba velocidad";
